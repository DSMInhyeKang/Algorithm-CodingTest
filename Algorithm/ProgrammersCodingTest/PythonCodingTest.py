...
# 등산코스 정하기(118669) - Lv.3
def solution(n, paths, gates, summits):
    field = {}
    intensity = [10000001] * (n+1)
    check, stack = set(summits), gates

    for A, B, time in paths:
        for x, y in ((A, B), (B, A)):
            try: field[x].append((y, time))
            except: field[x] = [(y, time)]

    for g in gates: intensity[g] = 0
        ...
        while stack:
            ...
            for to, time in field[now]:
                ...
                if intensity[to] > max_time:
                    ...
                    if to not in check: target.add(to)
...
def solution(a):
    result = [False] * len(a)
    rear, front = float('inf'), float('inf')
...
def solution(N, number):
    ...
    for i in range(len(arr)): arr[i].add(int(str(N) * (i+1)))
...
def solution(beginning, target):
    ...
    for i in range(1, m):
        if table[i] != table[0]:
            ...
            if (list(map(lambda x: x^1, table[i])) != table[0]): return -1
